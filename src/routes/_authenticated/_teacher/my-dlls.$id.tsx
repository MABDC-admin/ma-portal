...
function MyDllDetail() {
  ...
  if (q.isLoading) return <AppShell><p className="text-tertiary">Loading…</p></AppShell>;
  if (q.error || !q.data) return <AppShell><p className="text-status-absent">Not found or you don't have access.</p></AppShell>;
  ...
  function duplicate() {
    ...
    try {
      sessionStorage.setItem("dll:prefill", JSON.stringify({
        subject: d.subject,
  ...
  return (
    <AppShell>
      <header className="mb-6">
        <Link to="/my-dlls" className="mb-3 flex items-center gap-1 text-sm text-tertiary hover:text-foreground">
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {status === "returned" && (
              <button onClick={duplicate} className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow-sm hover:brightness-110">
      ...
      {d.feedback && (status === "returned" || status === "approved") && (
        <Card className={`mb-6 p-5 ${status === "returned" ? "border border-status-absent/30 bg-status-absent/5" : "border border-status-present/30 bg-status-present/5"}`}>
          <div className="flex items-start gap-3">
            <Icon name={status === "returned" ? "assignment_return" : "check_circle"} filled className={status === "returned" ? "text-status-absent" : "text-status-present"} />
            <div className="flex-1">
              <p className={`text-xs font-bold uppercase tracking-widest ${status === "returned" ? "text-status-absent" : "text-status-present"}`}>
                Director's feedback
              </p>
              <p className="mt-1 whitespace-pre-wrap text-sm">{d.feedback}</p>
              {d.reviewed_at && <p className="mt-2 text-xs text-tertiary num">{new Date(d.reviewed_at).toLocaleString()}</p>}
            </div>
          </div>
        </Card>
      )}
...
function Section({ title, body }: { title: string; body: string | null }) {
  return (
    <Card className="p-6">
      ...
      <p className="whitespace-pre-wrap text-sm leading-relaxed">{body || <em className="text-tertiary">Not provided.</em>}</p>
    </Card>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === "submitted") return <StatusPill tone="late" icon="pending">Awaiting review</StatusPill>;
